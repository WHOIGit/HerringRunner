...
ap.add_argument("-b", "--buffer-size", type=int, default=35,
...
def extractImages(video_source_path,seconds):
    ...
    while res:
      ...
      cv2.imwrite("{}_MSECS_{}.png".format(os.path.basename(video_source_path),str(count)), image)     # save frame as PNG file
...
while not args["debugger"]:
    ...
    avgsurf = (currentsurface*100)/surface #Calculate the average of contour area on the total size
